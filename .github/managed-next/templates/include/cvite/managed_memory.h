...
/*
 * Internal development-runtime ABI. Ordinary C application source does not
 * call these functions. Compiler instrumentation and the CVite host use this
 * layer only for allocations whose provenance remains controlled.
 */
...
/*
 * The caller must hold CVite's process-wide quiescence gate. The function
 * stages every replacement first and publishes pointer/address changes only
 * after every migrator invocation succeeds.
 */
